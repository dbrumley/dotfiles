...
says to load the rest from my org file. Note that the =:tangle no=
says that the above line is *not* exported as source as this is just
an example. Future lines will not have that directive.
...
We use orgmode here, which should load in a good version. I also
configure the default directory for org files. I use dropbox so I can
have access to my org files on any computer.
...
Let's make sure we can use pandoc to export orgmode files. pandoc is
extremely versatile, and can export to many different file formats.
...
* TODO
** TODO Try out helm 
** TODO Investigate load errors in dbrumley.org
These two warnings:
